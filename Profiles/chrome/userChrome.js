...
// @version       2021/08/05 fix for 92+ port Bug 1723723 - Switch JS consumers from
// getURLSpecFromFile to either getURLSpecFromActualFile or getURLSpecFromDir
// @version       2021/06/25 skip for in-content dialog etc.
// @version       2019/12/11 fix for 73 Bug 1601094 - Rename remaining .xul files to .xhtml in
// browser and Bug 1601093 - Rename remaining .xul files to .xhtml in toolkit Date 2019/12/11 01:30
// fix 72 revert the code for sidebar, use "load" in config.js(2019/12/11 01:30), Date 2019/08/11
// 21:30 fix 70.0a1  Bug 1551344 - Remove XULDocument code Date 2019/05/21 08:30 fix 69.0a1 Bug
// 1534407 - Enable browser.xhtml by default, Bug 1551320 - Replace all CreateElement calls in XUL
// documents with CreateXULElement Date 2018/08/10 01:30 fix 63.0a1 Date 2018/08/02 19:30 for
// userChrome.xml Date 2018/05/30 18:00 ALWAYSEXECUTE  .uc.js Date 2018/05/06 22:00 fix wrong commit
...
// Date 2010/10/10 00:00 Bug 377498 mozIJSSubscriptLoader::loadSubScript charset
// 入ったけどメタデータ // @charset  UTF-8 としとけばUTF-8で読み込む Date 2010/03/31 00:00
// XULDocumentのみに適用 Date 2010/03/11 17:30 debugbuildで動かない場合がある件に対応。 Date
// 2010/02/28 13:00 ↓が直っているので元に戻した。 Date 2009/08/06 00:00
// tree_style_tab-0.8.2009073102があるとxulのdocument.overlayが出来なくなる件に対応 Date 2009/05/23
// 00:00 userChrome.js0.8.1実験中 v3.0.25mod Date 2009/04/13 00:00 overlayのobserveの処理変更
// v3.0.24mod Date 2009/03/10 00:00 例外トラップ Date 2009/02/15 15:00
// chromehiddenなwindow(popup等)の場合にロードするかどうかを指定できるようにした。 Date 2008/12/29
// 06:00 面倒だからdocument.overlayを置き換えるようにした。 Date 2008/12/27 18:00
// Webpanelにchromeを読み込んだときのエラーが出るのを修正(thanks 音吉) Date 2008/09/16 00 00
// 面倒だからFirefox3 の場合はeditBookmarkOverlay.xulは先読みするように修正 Date 2008/08/28 00 00
// なぜか0.8.0+を使っている人がいたので, それに対応 Date 2008/08/26 23:50 08/26 18:00 以降
// Fx2で動かなくなったようなので, 直した Date 2008/08/26 22:00
// v3.0.11modで動かないなら,これ以降のものも動かないよ。たぶん Date 2008/08/26 18:00
// Fx3のStarUIをなんとかして欲しいな。 Date 2008/08/18 04:00 AUTOREMOVEBOM =
// trueなら文字コード自動判定するようにした。 Date 2008/08/16 15:00
// BOMを自動的に取り除くかどうか指定できるようにした Date 2008/07/29 23:00 なんかバグあったかも Date
// 2008/07/25 00:00 USE_0_63_FOLDERおよびFORCESORTSCRIPTがtrueの場合は,
// フォルダも名順でソートするようにした Date 2008/07/14 01:00 typo, regression Date 2008/07/14 00:00
// typo, regression Date 2008/07/13 22:00
// サイドバーweb-panelsにchromeウインドウを読み込んだ場合に対応 Date 2008/03/23 12:00
// 80氏のフォルダ規則に対応, 0.8modバージョンにも対応
//

(function() {
'use strict';
var {AppConstants} =
    AppConstants || ChromeUtils.importESModule('resource://gre/modules/AppConstants.sys.mjs');
// -- config --
const EXCLUDE_CHROMEHIDDEN =
    false;  // chromehiddenなwindow(popup等)ではロード: しないtrue, する[false]
const USE_0_63_FOLDER = false;  // 0.63のフォルダ規則を使う[true], 使わないfalse
const FORCESORTSCRIPT = (AppConstants.platform != 'win') ?
    true :
    false;  // 強制的にスクリプトをファイル名順でソートするtrue, しない[false]
const REPLACECACHE = true;  // スクリプトの更新日付によりキャッシュを更新する: true , しない:[false]
//=====================USE_0_63_FOLDER = falseの時===================
var UCJS = new Array(
    'UCJSFiles', 'userContent',
    'userMenu');  // UCJS Loader 仕様を適用 (NoScriptでfile:///を許可しておく)
var arrSubdir = new Array(
    '', 'xul', 'TabMixPlus', 'withTabMixPlus', 'SubScript', 'UCJSFiles', 'userCrome.js.0.8',
    'userContent', 'userMenu');  // スクリプトはこの順番で実行される
//===================================================================
const ALWAYSEXECUTE =
    ['rebuild_userChrome.uc.xul', 'rebuild_userChrome.uc.js'];  // 常に実行するスクリプト
var INFO = true;
var BROWSERCHROME =
    'chrome://browser/content/browser.xhtml';  // Firfox
...
window.userChrome_js = {
    ...
    get hackVersion() {
        ...
        if (!/^(chrome:\/\/\S+\/content\/)\S+/i.test(Error().fileName)) {
            ...
            'chrome://userchrome_js_cache/content/userChrome.js' ==
                Error().fileName) {  // 0.8.0+ or 0.7
    ...
    getScripts: function() {
        ...
        this.dirDisable =
            restoreState(getPref('userChrome.disable.directory', 'str', '').split(','));
        this.scriptDisable =
            restoreState(getPref('userChrome.disable.script', 'str', '').split(','));
        this.scripts  = [];
        this.overlays = [];
        ...
        function getScriptData(aContent, aFile) {
            ...
            return {
                filename: aFile.leafName, file: aFile, url: url,
                    // namespace: "",
                    charset: charset, description: description, async: async, sandbox: sandbox,
                    // code: aContent.replace(header, ""),
                    regex: new RegExp(
                        '^' + exclude + '(' + (rex.include.join('|') || '.*') + ')$', 'i')
            }
        }
        ...
        function readFile(aFile, metaOnly) {
            ...
            cvstream.init(
                stream, 'UTF-8', 1024, Ci.nsIConverterInputStream.DEFAULT_REPLACEMENT_CHARACTER);
            var content = '', data = {};
        ...
        function writeFile(aFile, aData) {
            ...
            foStream.init(
                aFile, 0x02 | 0x08 | 0x20, parseInt(664, 8), 0);  // write, create, truncate
        ...
        function getPref(aPrefString, aPrefType, aDefault) {
            ...
            try {
                switch (aPrefType) {
                    case 'complex':
                        return xpPref.getComplexValue(
                            aPrefString, Components.interfaces.nsILocalFile);
                        break;
                    case 'str':
    ...
    runOverlays: function(doc) {
        ...
        for (var m = 0, len = this.overlays.length; m < len; m++) {
            ...
            if (this.ALWAYSEXECUTE.indexOf(overlay.filename) < 0 &&
                (!!this.dirDisable['*'] || !!this.dirDisable[overlay.dir] ||
            ...
            if (overlay.regex.test(dochref)) {
                ...
                this.loadOverlay(
                    overlay.url + '?' + this.getLastModifiedTime(overlay.file), null, doc);
            }
        }
    },
    ...
    runScripts: function(doc) {
        ...
        for (var m = 0, len = this.scripts.length; m < len; m++) {
            ...
            if (this.ALWAYSEXECUTE.indexOf(script.filename) < 0 &&
                (!!this.dirDisable['*'] || !!this.dirDisable[script.dir] ||
            ...
            if (script.ucjs) {  // for UCJS_loader
                ...
                */
                ...
                if (!script.async) {
                    ...
                    ChromeUtils
                        .compileScript(script.url + '?' + this.getLastModifiedTime(script.file))
                        .then((r) => {
                            r.executeInGlobal(/*global*/ script.sandbox ? target : doc.defaultView,
    ...
    error: function(aMsg, err) {
        ...
        if (typeof (err) == 'object')
            error.init(
                aMsg + '\n' + err.name + ' : ' + err.message, err.fileName || null, null,
                err.lineNumber, null, 2, err.name);
        else
...
if (location.href === that.BROWSERCHROME && that.EXCLUDE_CHROMEHIDDEN &&
...
var loadInWebpanel = {
    ...
    init: function(sidebarWindow) {
        ...
        this.sidebarWindow.document.getElementById('web-panels-browser')
